...
ggplot2::ggplot(dplyr::mutate(koala.df, 
                              binary_response = dplyr::case_when(binary_response == 0 ~ "No",
                                                                 binary_response == 1 ~ "Yes",
                                                                 TRUE ~ "unknown")), 
                aes(fill = as.factor(binary_response), x = image_id)) + 
...
## Basic visualisation of data
ggplot2::ggplot(dplyr::mutate(reef.df,  
                              binary_response = dplyr::case_when(binary_response == 0 ~ "No",
                                                                 binary_response == 1 ~ "Yes",
                                                                 TRUE ~ "unknown")), 
                aes(fill = as.factor(binary_response), x = image_id)) + 
...
if(nrow(jaguar.df) > 1){
  ...
  ggplot2::ggplot(jaguar.df,aes(x = image_id, fill = response)) +
    ...
    # if() {facet_grid(cols = vars(image_id), scales = "free")} +
    theme_bw() +
    labs(fill = "Observed\nhabitat\nfeatures") +
