...
2. Generating a list of entity HTML elements, one for every data point.
...
a_scatterplot <- function(json_data, x, y, z, ...){
  ...
  ## Write JSON data
  json_file_asset <- a_asset(id = "scatterdata",
                             src = json_data)
...
jsonlite::write_json(diamonds, json_file) 

my_scene <- a_scene(template = "empty",
                    children = list(
                      a_scatterplot(json_file, x = "depth", y = "carat", z = "table",
                                    val = "price",
